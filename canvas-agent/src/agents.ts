...
function codexEventScope(params: Json) {
  const threadId = String(
    field(params, "threadId") || field(field(params, "thread"), "id") || "",
  );
  const turnId = String(
    field(params, "turnId") || field(field(params, "turn"), "id") || "",
  );
...
async function writeAttachmentFile(item: AgentAttachment) {
  ...
  const file = path.join(
    os.tmpdir(),
    `artboard-flow-${Date.now()}-${Math.random().toString(16).slice(2)}.${imageExt(meta || item.type)}`,
  );
