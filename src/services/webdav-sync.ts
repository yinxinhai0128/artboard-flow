...
export async function downloadWebdavFile(
  ...
  if (!response.ok)
    await throwWebdavError(response, "读取 WebDAV 同步文件失败");
...
export async function uploadWebdavFile(
  ...
  if (!response.ok)
    await throwWebdavError(response, "上传 WebDAV 同步文件失败");
}
...
async function webdavFetch(
  ...
  try {
    ...
    if (error instanceof Error && error.name === "AbortError")
      throw new Error("WebDAV 请求超时，请检查网络或远端服务状态");
...
async function throwWebdavError(
  ...
  if (response.status === 404)
    throw new Error("WebDAV 路径不存在，请检查地址和远程目录");
  throw new Error(
    `${fallback}：${response.status}${detail ? ` ${detail.slice(0, 120)}` : ""}`,
  );
}
