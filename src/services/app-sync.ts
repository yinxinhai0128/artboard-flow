...
export async function syncAppDataToWebdav(
  ...
  const [canvas, assets, imageLogs, videoLogs] = await Promise.all([
    ...
    syncDomain<AssetDomainData>(config, onProgress, {
      ...
      applyData: async (data) =>
        useAssetStore
          .getState()
          .replaceAssets(await Promise.all(data.assets.map(hydrateAsset))),
    }),
  ...
  const result = {
    ...
    mergedRemote: [canvas, assets, imageLogs, videoLogs].some(
      (item) => item.mergedRemote,
    ),
...
async function syncDomain<T>(
  ...
  try {
    ...
    const remoteManifest = await readDomainManifest(
      config,
      options.key,
      options.emptyData,
    );
...
async function readDomainManifest<T>(
  ...
  const file = await downloadWebdavFile(
    config,
    domainPath(domain, WEBDAV_MANIFEST_FILE_NAME),
  );
  if (!file) return null;
  const data = JSON.parse(await file.text()) as DomainManifest<T>;
  if (data.app !== "artboard-flow" || data.domain !== domain)
...
async function downloadMissingFiles<T>(
  ...
  const remoteFileMap = new Map(
    remoteFiles.map((item) => [item.storageKey, item]),
  );
  ...
  await runWithConcurrency(tasks, FILE_CONCURRENCY, async (remoteFile) => {
    ...
    const typedBlob = blob.type
      ? blob
      : blob.slice(0, blob.size, remoteFile.mimeType);
...
async function uploadChangedFiles<T>(
  ...
  for (const storageKey of storageKeys) {
    ...
    files.push(item);
    if (!remoteFile || remoteFile.bytes !== blob.size)
...
async function hydrateAsset(asset: Asset): Promise<Asset> {
  if (asset.kind === "image" && asset.data.storageKey) {
    const dataUrl = await resolveImageUrl(
