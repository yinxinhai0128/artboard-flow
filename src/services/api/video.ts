...
export type VideoGenerationTask = {
  id: string;
  provider: "openai" | "seedance" | "gateway" | "plugin";
  model: string;
};
...
export async function createVideoGenerationTask(
  ...
  if (isGatewayBaseUrl(requestConfig.baseUrl)) {
    return createGatewayTask(
      requestConfig,
      selectedModel,
      prompt,
      references,
...
export async function pollVideoGenerationTask(
  ...
  if (task.provider === "plugin") {
    ...
    return result
      ...
      : { status: "failed", error: "插件视频任务已失效，请重新生成" };
  }
  const requestConfig = resolveModelRequestConfig(config, task.model);
  assertVideoConfig(requestConfig, requestConfig.model);
  if (task.provider === "seedance")
    return pollSeedanceTask(requestConfig, task, options);
  if (task.provider === "gateway")
    return pollGatewayTask(requestConfig, task, options);
...
async function createSeedanceTask(
  ...
  const { referenceLimits } = seedanceModelProfile(modelName);
  if (
    audioReferences.length &&
    !references.length &&
    !videoReferences.length &&
    !referenceLimits.audioOnlyAllowed
  ) {
    throw new Error(
      "Seedance 参考音频不能单独使用，请同时添加参考图或参考视频",
    );
  }
  assertSeedanceVideoReferences(videoReferences, referenceLimits);
  assertSeedanceAudioReferences(audioReferences, referenceLimits);
...
async function createGatewayTask(
  config: AiConfig,
  model: string,
  prompt: string,
  references: ReferenceImage[],
  ...
  try {
    const created = (
      await axios.post<{ id?: string; task_id?: string }>(
    ...
    if (!id) throw new Error("网关没有返回任务 ID");
    return { id, provider: "gateway", model };
  } catch (error) {
    throw new Error(readAxiosError(error, "网关视频任务创建失败"));
  }
}

async function pollGatewayTask(
  config: AiConfig,
  task: VideoGenerationTask,
  options?: RequestOptions,
): Promise<VideoGenerationTaskState> {
  try {
    ...
    if (state?.status === "completed" || state?.status === "succeeded")
      return { status: "failed", error: "网关任务完成但没有返回视频 URL" };
    if (
      state?.status === "failed" ||
      state?.status === "cancelled" ||
      state?.status === "error"
      ...
      return {
        status: "failed",
        error: state?.error?.message || "网关视频生成失败",
      };
    return { status: "pending" };
  } catch (error) {
    throw new Error(readAxiosError(error, "网关视频任务查询失败"));
  }
}

function assertSeedanceVideoReferences(
  videoReferences: ReferenceVideo[],
  limits: SeedanceReferenceLimits,
) {
  const error = seedanceVideoReferenceError(videoReferences, limits);
  if (error) throw new Error(error);
}

function assertSeedanceAudioReferences(
  audioReferences: ReferenceAudio[],
  limits: SeedanceReferenceLimits,
) {
  const error = seedanceAudioReferenceError(audioReferences, limits);
  if (error) throw new Error(error);
}
...
async function resolveSeedanceVideoUrl(video: ReferenceVideo) {
  ...
  if (!blob)
    throw new Error("参考视频必须是公网 URL、资产 ID，或本地已保存的视频");
...
async function resolveSeedanceAudioUrl(audio: ReferenceAudio) {
  ...
  if (!blob)
    throw new Error("参考音频必须是公网 URL、资产 ID，或本地已保存的音频");
  return blobToDataUrl(blob);
}
...
function readApiErrorMessage(value: unknown): string {
  ...
  return (
    ...
    readApiErrorMessage(payload.error?.message)
  );
}
...
function statusMessage(status: number | undefined, fallback: string) {
  if (status === 401 || status === 403)
    return "鉴权失败，请检查 API Key、套餐权限或模型权限";
...
async function assertVideoBlob(blob: Blob) {
  ...
  if (typeof payload.code === "number" && payload.code !== 0)
    throw new Error(readApiErrorMessage(payload) || "视频下载失败");
