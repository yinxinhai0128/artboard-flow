...
export async function runPromptSource(
  ...
  if (!Array.isArray(result))
    throw new Error("提示词来源脚本需要 return 一个数组");
  ...
  for (const raw of result) {
    ...
    const id =
      String(record.id || "").trim() || `prompt-${leftPad(items.length + 1)}`;
...
export const PROMPT_SOURCE_VARIABLES: PromptSourceVariable[] = [
  {
    name: "fetchText",
    type: "function",
    desc: "fetchText(url) 拉取纯文本（README 等），失败抛错",
  },
  {
    name: "fetchJson",
    type: "function",
    desc: "fetchJson(url) 拉取并解析 JSON",
  },
  {
    name: "splitSections",
    type: "function",
    desc: "splitSections(markdown, prefix) 按标题前缀（如 '### '）切分成段落数组",
  },
  {
    name: "firstMatch",
    type: "function",
    desc: "firstMatch(text, /正则/) 返回第一个捕获组，未匹配返回空串",
  },
  {
    name: "extractImages",
    type: "function",
    desc: "extractImages(baseUrl, markdown) 提取 markdown 图片并补全为绝对地址",
  },
  {
    name: "absoluteUrl",
    type: "function",
    desc: "absoluteUrl(baseUrl, path) 把相对路径拼成绝对 URL",
  },
  {
    name: "tagsFromHeading",
    type: "function",
    desc: "tagsFromHeading(heading) 从标题按 / & 、与 切出标签（小写去重前）",
  },
  {
    name: "splitTags",
    type: "function",
    desc: "splitTags(value, /分隔符/) 切分标签并转小写去空",
  },
  {
    name: "markdownPreview",
    type: "function",
    desc: "markdownPreview(images) 把图片数组拼成 markdown 预览文本",
  },
  {
    name: "leftPad",
    type: "function",
    desc: "leftPad(n) 数字左补零到 4 位，用于生成有序 id",
  },
  {
    name: "makePrompt",
    type: "function",
    desc: "makePrompt({id,title,prompt,coverUrl,tags,preview}) 构造一条提示词；title 和 prompt 必填",
  },
