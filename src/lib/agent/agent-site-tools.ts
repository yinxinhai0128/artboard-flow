...
function getGenerationStatus(
  ...
  const nodeIds = new Set(
    Array.isArray(input.nodeIds)
      ? input.nodeIds.filter((id): id is string => typeof id === "string")
      : [],
  );
  const limit = Math.max(
    1,
    Math.min(100, Math.floor(Number(input.limit)) || 20),
  );
  const tasks: GenerationStatusItem[] = [];
  const includeCanvas =
    (scope === "all" || scope === "canvas") && (!taskId || nodeIds.size > 0);
  ...
  if (includeWorkbench) {
    useWorkbenchAgentStore.getState().tasks.forEach((task) => {
      if ((scope === "image" || scope === "video") && task.kind !== scope)
        return;
  ...
  tasks.sort(
    (a, b) =>
      generationStatusOrder(a.status) - generationStatusOrder(b.status) ||
...
function normalizeCanvasGenerationStatus(
  status: unknown,
...
function getImageConfig() {
  ...
  return {
    ...
    models: selectableModelsByCapability(config, "image").map((value) => ({
      value,
...
function runImageWorkbench(input: SiteToolInput, navigate: NavigateFunction) {
  ...
  if (typeof input.model === "string" && input.model.trim()) {
    const value =
      normalizeModelOptionValue(input.model, configStore.config.channels) ||
      input.model;
  ...
  if (input.count != null) {
    const count = String(
      Math.max(1, Math.min(15, Math.floor(Number(input.count)) || 1)),
    );
  ...
  navigate("/image");
  const taskId = useWorkbenchAgentStore
    .getState()
    .dispatchImage({ prompt, run });
  return {
    ok: true,
    navigated: "/image",
    prompt,
...
function getVideoConfig() {
  ...
  return {
    ...
    models: selectableModelsByCapability(config, "video").map((value) => ({
      value,
      label: modelOptionLabel(config, value),
    })),
...
async function searchPrompts(input: SiteToolInput) {
  ...
  const result = await fetchPrompts({
    ...
    category: String(input.category || "全部"),
    tag: tags,
    page,
    pageSize,
  });
...
function listAssets(input: SiteToolInput) {
  ...
  const kind =
    input.kind === "text" || input.kind === "image" || input.kind === "video"
      ? input.kind
      : "all";
  const keyword = String(input.keyword || "")
    .trim()
  ...
  const filtered = assets.filter((asset) => {
    ...
    return [asset.title, asset.note, asset.source, ...asset.tags]
      .filter(Boolean)
      .join(" ")
      .toLowerCase()
      .includes(keyword);
...
async function addAsset(input: SiteToolInput) {
  ...
  if (kind === "image") {
    ...
    if (!imageUrl)
      throw new Error("kind=image 时需要提供 imageUrl（图片地址或 dataURL）");
    let stored;
    try {
...
function paginate(input: SiteToolInput, total: number, defaultSize: number) {
  const pageSize = Math.max(
    1,
    Math.min(100, Math.floor(Number(input.pageSize)) || defaultSize),
  );
