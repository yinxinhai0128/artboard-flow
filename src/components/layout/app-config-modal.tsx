...
export function AppConfigPanel({
  ...
  const addChannel = () => {
    const channel = createModelChannel({
      name: `渠道 ${config.channels.length + 1}`,
    });
  ...
  const testWebdav = async () => {
    ...
    try {
      ...
      message.error(
        error instanceof Error ? error.message : "WebDAV 连接测试失败",
      );
    } finally {
  ...
  const updateWebdavProgress = (event: AppSyncProgressEvent) => {
    ...
    setWebdavDomainProgress((current) => ({
      ...
      [event.domain as AppSyncDomainKey]: {
        label:
          event.label || webdavDomainLabels[event.domain as AppSyncDomainKey],
        stage: event.stage,
  ...
  const syncWebdav = async () => {
    ...
    try {
      ...
      message.success(
        `同步完成：${result.projects} 个画布，${result.assets} 个资产，${result.imageLogs + result.videoLogs} 条记录，本次上传 ${result.uploadedFiles} 个文件 ${formatBytes(result.uploadedBytes)}`,
      );
    } catch (error) {
      setWebdavSyncStatus(
        error instanceof Error ? error.message : "WebDAV 同步失败",
      );
                  ...
                  <div className="mb-3 flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <div className="mt-1 text-xs text-stone-500">
                        同步画布、我的资产、生成记录和本地媒体文件，不包含 AI
                        API Key；浏览器会直接连接 WebDAV 服务。
                      </div>
                    </div>
                    <div className="text-xs text-stone-500">
                      {webdav.lastSyncedAt
