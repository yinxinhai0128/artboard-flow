...
import {
  ...
  uninstallPlugin,
  updatePlugin,
} from "@/lib/canvas/plugin-loader";
import {
  fetchOfficialPlugins,
  hasUpgrade,
  type OfficialPluginEntry,
} from "@/lib/canvas/plugin-registry";
import { useThemeStore } from "@/stores/use-theme-store";
import {
  usePluginStore,
  type InstalledPlugin,
} from "@/stores/canvas/use-plugin-store";
...
export function CanvasPluginManagerModal({
  ...
  const tabs = [
    ...
    ...(localPlugins.length > 0
      ? [{ key: "local", label: "本地插件", children: localTab }]
      : []),
    ...
    >
      <div className="space-y-3">
        ...
        >
          ...
          <span>
            插件代码会在当前页面内直接执行，可访问本地数据（包含 AI API
            Key）。请仅安装你信任来源的插件。
          </span>
        </div>
