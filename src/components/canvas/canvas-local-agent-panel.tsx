...
import {
  ...
  runSiteTool,
  SITE_TOOL_LABELS,
} from "@/lib/agent/agent-site-tools";
import {
  AgentChatComposer,
  AgentChatMessage,
  AgentPanelTabs,
...
const AGENT_CONNECT_STEPS = [
  {
    title: "方式一：在 Codex 中使用插件",
    text: "在 Codex app 安装 ArtboardFlow 插件后，通过插件启动画布，插件会自动启动本地 Agent 并带上连接信息。",
  },
  {
    title: "方式二：直接运行 Agent",
    text: "不使用 Codex 插件时，在终端运行下面命令，再回到网页里连接或手动填入 Local URL 和 Connect token。",
    command: "npx -y @yinxinhai0128/artboard-flow-agent",
  },
];
const AGENT_PLUGIN_REMOVE_COMMAND = "codex plugin remove artboard-flow";
const AGENT_MCP_REMOVE_COMMAND = "codex mcp remove artboard-flow";
...
export function CanvasLocalAgentPanel({
  ...
  const canvasContextRef = useRef<AgentCanvasContext | null>(
    useAgentStore.getState().canvasContext,
  );
  ...
  const urlAgentAutoConnect =
    searchParams.has("agentUrl") && searchParams.has("agentToken");
      ...
      try {
        ...
        if (nextThreadId && !skipHistory) {
          const thread = await fetchAgentJson<AgentThreadResponse>(
            endpoint,
            token,
            `/agent/codex/threads/${encodeURIComponent(nextThreadId)}`,
          );
      ...
      timer = setTimeout(
        () =>
          void postState(
            ...
            token,
            clientIdRef.current,
            canvasContextRef.current?.snapshot || null,
          ),
        300,
      );
    });
  ...
  useEffect(() => {
    ...
    const enqueueEvent = (task: () => void | Promise<void>) => {
      eventQueue = eventQueue
        .then(task)
        .catch((error) => addEventLog("同步会话失败", error));
    };
    ...
    source.addEventListener("hello", (event) => {
      ...
      if (document.visibilityState === "visible" && document.hasFocus())
        void activateAgentClient(endpoint, token, clientId);
    });
    ...
    source.onerror = () => {
      ...
      const text = wasConnected
        ? "本地 Agent 连接失败或已断开"
        : "连接失败，请检查地址和 token";
      if (!errorLoggedRef.current || wasConnected) {
        addEventLog(wasConnected ? "连接断开" : "连接失败", {
          endpoint,
          error: text,
        });
  ...
  const sendPrompt = async () => {
    ...
    try {
      const data = await fetchAgentJson<{ threadId?: string }>(
        endpoint,
        token,
        "/agent/codex/turn",
        {
          ...
          body: JSON.stringify({
            ...
            attachments: files.map(({ name, type, dataUrl }) => ({
              name,
              type,
              dataUrl,
            })),
  ...
  const stopTurn = async () => {
    ...
    try {
      await fetch(
        `${endpoint}/agent/codex/interrupt?token=${encodeURIComponent(token)}`,
        {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({
  ...
  const runToolCall = async (
    ...
    if (isSiteTool(payload.name)) {
      try {
        ...
        addMessage({
          ...
          text: siteToolSummary(payload.name, result),
          detail: {
            requestId: payload.requestId,
            name: payload.name,
            input: payload.input,
    ...
    try {
      const input: { ops?: CanvasAgentOp[]; path?: string } =
        payload.input || {};
      addEventLog(toolName(payload.name), payload, payload);
      let result: unknown;
      if (payload.name === "site_navigate") {
        ...
        if (!context)
          throw new Error("当前不在画布页，请先用 site_navigate 打开画布");
        result = context.applyOps(input.ops || []);
        void postState(
          endpoint,
          token,
          clientIdRef.current,
          result as CanvasAgentSnapshot,
        );
      } else {
  ...
  useEffect(() => {
    if (urlAgentAutoConnect && confirmTools)
  ...
  const deleteThread = async (threadId: string) => {
    ...
    try {
      ...
      setAgentState({
        ...
        activeThreadId:
          current.activeThreadId === threadId ? "" : current.activeThreadId,
  ...
  const addMessage = (item: Omit<AgentChatItem, "id"> & { id?: string }) => {
    ...
    if (
      ...
      setAgentState({
        messages: [
          ...useAgentStore.getState().messages.slice(0, -1),
  ...
  const handleAgentEvent = (event: AgentEventPayload) => {
    if (shouldLogAgentEvent(event))
      addEventLog(eventTitle(event), event, event);
        ...
        right={
          <>
            <Button
              size="small"
              type="text"
              disabled={!connected || loadingThreads || sending || waiting}
      ...
      {activeTab === "setup" ? (
        ...
        <>
          ...
          >
            ...
            {waiting && !pendingTool ? (
              <AgentWorkingMessage theme={theme} />
            ) : null}
          </div>
          <AgentChatComposer
            prompt={prompt}
...
function AgentLogView({
  ...
  const content =
    mode === "text"
      ? formatLogText(logs, context)
      : formatLogJson(logs, context);
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              onClick={() =>
                lastError &&
                void copy(formatLogText([lastError], context), "最近错误已复制")
              }
            >
              最近错误
            </Button>
...
function AgentConnectView({
  ...
  const codexPluginReminder = (
    ...
    >
      ...
      <div className="mt-1">
        只有安装 Codex 插件或手动添加 MCP 后，工具列表才会进入 Codex
        上下文并增加 token 消耗；仅运行 `npx -y
        @yinxinhai0128/artboard-flow-agent` 启动本地 Agent 不会安装 MCP。
      </div>
      <div className="mt-2 grid gap-1.5">
        {[
          ...
          <div
            key={command}
            className="flex items-center gap-2 rounded-md border bg-transparent px-2 py-1.5"
            style={{ borderColor: theme.node.stroke, color: theme.node.text }}
          >
            <span
...
async function activateAgentClient(
  endpoint: string,
  token: string,
  clientId: string,
) {
...
function formatAgentEvent(
  ...
  if (text)
    return { role: "assistant", title: "Codex", text, meta: usageText(event) };
  return null;
}
...
function isCurrentThreadEvent(event: {
  threadId?: string;
  thread_id?: string;
}) {
  const threadId = event.threadId || event.thread_id || "";
  return (
    Boolean(threadId) && threadId === useAgentStore.getState().activeThreadId
  );
}

function formatLogText(logs: AgentEventLog[], context: AgentLogContext) {
  const head = [
    "ArtboardFlow Agent 诊断日志",
  ...
  const body = logs
    .map((item, index) => {
      const detail =
        item.raw == null ? item.text : JSON.stringify(item.raw, null, 2);
      return [`#${index + 1} ${item.time} ${item.title}`, detail]
        .filter(Boolean)
        .join("\n");
    })
    .join("\n\n---\n\n");
...
function formatLogJson(logs: AgentEventLog[], context: AgentLogContext) {
  return JSON.stringify(
    {
      context,
      logs: logs.map(({ time, title, text, raw }) => ({
        time,
        title,
        text,
        raw,
...
function isConnectionErrorMessage(item: AgentChatItem) {
  return (
    item.role === "error" &&
    /连接失败|无法连接本地 Agent|本地 Agent 连接失败/.test(item.text)
  );
}
...
function siteToolSummary(name: string, result: unknown) {
  ...
  if (name === "canvas_list_projects")
    return `共 ${numberField(data, "total")} 个画布`;
  if (name === "prompts_search")
    return `找到 ${numberField(data, "total")} 条提示词`;
  ...
  if (
    ...
    return typeof data.note === "string" ? data.note : "已在工作台执行";
  if (
