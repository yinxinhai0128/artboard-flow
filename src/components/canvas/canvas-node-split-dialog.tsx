...
export function CanvasNodeSplitDialog({
  ...
  const addLine = (axis: "horizontal" | "vertical") => {
    setParams((current) => {
      ...
      const lines = [
        ...(current[key] || []),
        findLineSpot(current[key] || []),
      ].sort((a, b) => a - b);
      return {
        ...current,
        [key]: lines,
        rows: axis === "horizontal" ? lines.length + 1 : current.rows,
        columns: axis === "vertical" ? lines.length + 1 : current.columns,
      };
    });
  };
  const deleteLine = () => {
        ...
        <div className="grid gap-6 md:grid-cols-[minmax(260px,1fr)_280px]">
          <div className="rounded-xl border p-4">
            <div className="grid min-h-[300px] place-items-center rounded-lg bg-black/5">
              <div
                ref={previewRef}
                className="relative inline-block max-w-full overflow-hidden rounded-lg bg-black shadow-xl"
              >
                <img
                  src={dataUrl}
                  alt=""
