...
import {
  ...
  ConnectionPath,
} from "@/components/canvas/canvas-connections";
...
} from "@/components/canvas/canvas-node-angle-dialog";
import {
  CanvasNodeCropDialog,
  type CanvasImageCropRect,
} from "@/components/canvas/canvas-node-crop-dialog";
import {
...
} from "@/components/canvas/canvas-node-split-dialog";
import {
...
import {
  ...
  CanvasNodeInfoModal,
} from "@/components/canvas/canvas-node-hover-toolbar";
import { ArtboardFlow } from "@/components/canvas/artboard-flow";
import { Minimap } from "@/components/canvas/canvas-mini-map";
import { CanvasNode } from "@/components/canvas/canvas-node";
import {
  CanvasNodePromptPanel,
  type CanvasNodeGenerationMode,
} from "@/components/canvas/canvas-node-prompt-panel";
import { CanvasToolbar } from "@/components/canvas/canvas-toolbar";
import {
  AssetPickerModal,
  type InsertAssetPayload,
} from "@/components/canvas/asset-picker-modal";
...
export default function CanvasPage() {
  ...
  return <ArtboardFlowPage />;
}

function ArtboardFlowPage() {
  ...
  useEffect(() => {
    if (
      !projectLoaded ||
      !["new", "recent", "choose"].includes(searchParams.get("mode") || "")
    )
      return;
  ...
  const batchChildCountById = useMemo(() => {
    ...
    nodes.forEach((node) => {
      if (node.metadata?.isBatchRoot)
        map.set(node.id, node.metadata.batchChildIds?.length || 0);
    });
    return map;
  }, [nodes]);
  ...
  const maskEditImageNode = useCallback(
    async (node: CanvasNodeData, payload: CanvasImageMaskEditPayload) => {
      ...
      try {
        ...
        const errorDetails =
          error instanceof Error ? error.message : "局部修改失败";
    ...
    async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => {
      ...
      try {
        if (mode === "image") {
          ...
          const childIds =
            count > 1 ? Array.from({ length: count }, () => nanoid()) : [];
          ...
          const rootNode: CanvasNodeData = {
            ...
            position: {
              x: isEmptyImageNode
                ? parentPosition.x
                : parentPosition.x + parentConfig.width + gap,
              y:
                parentPosition.y +
          ...
          const childNodes: CanvasNodeData[] = childIds.map((id, index) => ({
            ...
            position: {
              x:
                ...
                rootNode.width +
                120 +
                (index % 2) * (imageConfig.width + 36),
              y:
                rootNode.position.y +
                Math.floor(index / 2) * (imageConfig.height + rowGap),
            },
            width: imageConfig.width,
            height: imageConfig.height,
            metadata: {
