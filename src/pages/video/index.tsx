...
type UpdateAiConfig = <K extends keyof AiConfig>(
  key: K,
  value: AiConfig[K],
) => void;

const LOG_STORE_KEY = "artboard-flow:video_generation_logs";
...
export default function VideoPage() {
  ...
  const clearVideoCommand = useWorkbenchAgentStore(
    (state) => state.clearVideoCommand,
  );
  ...
  useEffect(() => {
    if (!videoCommand || videoCommand.nonce === processedCommandRef.current)
      return;
    ...
    if (videoCommand.run && running) {
      if (videoCommand.taskId)
        updateAgentTask(videoCommand.taskId, {
          status: "failed",
          error: "视频工作台已有任务正在运行",
        });
      return;
  ...
  const pollGenerationLog = async (
    ...
    setResults((value) =>
      value.length ? value : [{ id: log.id, status: "pending" }],
    );
  ...
  const previewGenerationLog = (log: GenerationLog) => {
    ...
    if (log.config.videoSeconds)
      updateConfig("videoSeconds", log.config.videoSeconds);
    if (log.config.videoGenerateAudio)
      updateConfig("videoGenerateAudio", log.config.videoGenerateAudio);
    if (log.config.videoWatermark)
      updateConfig("videoWatermark", log.config.videoWatermark);
    setResults(
      log.status === "生成中"
...
async function normalizeLog(
  ...
  const video = log.video?.storageKey
    ? {
        ...log.video,
        url: await resolveMediaUrl(log.video.storageKey, log.video.url),
      }
    : log.video;
  const videoReferences = await Promise.all(
    (log.videoReferences || []).map(async (item) => ({
      ...item,
      url: item.storageKey
        ? await resolveMediaUrl(item.storageKey, item.url)
        : item.url,
    })),
  );
