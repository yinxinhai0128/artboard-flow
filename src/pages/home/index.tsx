import { ArrowRight, Clapperboard, ImagePlus, Layers, Play, Plus, Search, Sparkles, Wand2 } from "lucide-react";
...
const HERO_FEATURES = [
    ...
    { icon: ImagePlus, title: "AI 生图", desc: "Qwen-Image 多风格直出", slug: "image" },
    { icon: Layers, title: "无限画布", desc: "节点编排，连接创意", slug: "canvas" },
] as const;

const SHOW_TABS = ["全部", "精选", "广告设计", "人像", "3d", "风景", "动漫"] as const;
...
export default function IndexPage() {
    ...
    const [activeTab, setActiveTab] = useState<(typeof SHOW_TABS)[number]>("全部");
    ...
    const filtered = promptShowcase.filter((item) => {
        if (activeTab !== "全部" && !item.tags.some((tag) => tag.includes(activeTab))) return false;
        if (keyword && !item.title.includes(keyword) && !item.prompt.includes(keyword)) return false;
        return true;
    });
            ...
            <div className="mx-auto max-w-6xl px-6 pb-20 pt-10">
                ...
                <section className="relative overflow-hidden rounded-2xl border border-border bg-card px-8 py-14">
                    ...
                    <div className="relative">
                        <Tag className="mb-4 !border-foreground/15 !bg-foreground/[0.04] !text-muted-foreground">ArtboardFlow</Tag>
                        ...
                        <div className="mt-8 flex flex-wrap items-center gap-3">
                            <Button type="primary" size="large" onClick={() => navigate(`/${primaryTool.slug}`)} icon={<Plus className="size-4" />}>
                                新建画布创作
                            </Button>
                            <Button size="large" onClick={() => navigate("/image")} icon={<ImagePlus className="size-4" />}>
                                去生图
                            </Button>
                        </div>
                    </div>
                    ...
                    {HERO_FEATURES.map((feature) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="mt-1 text-xs leading-5 text-muted-foreground">{feature.desc}</div>
                            </button>
                        );
                    })}
                </section>
                ...
                <section className="mt-12">
                        ...
                        {SHOW_TABS.map((tab) => (
                            <button
                                ...
                                className={cn(
                                    ...
                                    activeTab === tab ? "bg-foreground text-background" : "bg-card text-muted-foreground hover:text-foreground",
                                )}
                            >
                                {tab}
                            </button>
                    ...
                    {filtered.length ? (
                            ...
                            {filtered.map((item) => (
                                ...
                                >
                                    <div className="relative aspect-video overflow-hidden bg-muted">
                                        <img src={item.coverUrl} alt={item.title} className="h-full w-full object-cover transition duration-500 group-hover:scale-[1.05]" />
                                    ...
                                    <div className="p-3">
                                        ...
                                        <div className="mt-1.5 flex items-center justify-between text-[11px] text-muted-foreground">
                                            <span className="line-clamp-1">{user?.username || "ArtboardFlow"}</span>
                        ...
                        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border py-16 text-center">
                            ...
                            <p className="text-sm text-muted-foreground">该分类下暂无提示词</p>
                        </div>
                    )}
                </section>
            </div>
