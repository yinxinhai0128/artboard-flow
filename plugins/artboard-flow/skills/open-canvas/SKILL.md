---
name: open-canvas
description: 打开 ArtboardFlow 在线或本地画布，并自动连接本地 Canvas Agent。用户要求打开、启动、进入或使用 ArtboardFlow 画布时使用。
---
...
1. 在 ArtboardFlow 项目中启动前端，并使用 Vite 输出的 `Local` 地址：
...
插件在新的 Codex 任务中加载时会自动启动 `npx -y @yinxinhai0128/artboard-flow-agent mcp`。这个 MCP 进程负责提供画布工具，不提供网页连接服务；
