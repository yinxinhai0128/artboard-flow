...
const JWT_SECRET = process.env.AUTH_JWT_SECRET || createHash('sha256').update('artboard-flow-local-secret').digest('hex')
...
export function signToken(payload) {
  ...
  const body = base64UrlEncode(JSON.stringify({ ...payload, iat: Math.floor(Date.now() / 1000), exp: Math.floor(Date.now() / 1000) + TOKEN_TTL_SECONDS }))
