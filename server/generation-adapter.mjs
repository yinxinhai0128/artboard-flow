export const GENERATION_JOB_STATUSES = new Set([
  "queued",
  "running",
  "succeeded",
  "failed",
...
export function normalizeGenerationJobPayload(input = {}) {
  ...
  const mode =
    payload.mode === "video" || payload.mode === "text"
      ? payload.mode
      : "image";
  const prompt = typeof payload.prompt === "string" ? payload.prompt : "";
  const inputs = Array.isArray(payload.inputs)
    ? payload.inputs.flatMap(normalizePayloadInput)
    : [];
  return {
    mode,
    generationType:
      payload.generationType === "edit"
        ? "edit"
        : payload.generationType === "generation"
          ? "generation"
          : undefined,
    model:
    ...
    count: Math.max(
      1,
      Math.min(15, Math.floor(Math.abs(Number(payload.count)) || 1)),
    ),
    prompt,
...
export function normalizeGenerationJobResult(input = {}) {
  ...
  const explicitOutputs = Array.isArray(result.outputs)
    ? result.outputs.flatMap(normalizeSingleGenerationOutput)
    : [];
  const primary = normalizeSingleGenerationOutput(result)[0] ??
    explicitOutputs[0] ?? {
      content: "",
      mimeType: undefined,
      bytes: undefined,
  ...
  return [
    {
      ...
      naturalWidth: Number.isFinite(result.naturalWidth)
        ? result.naturalWidth
        : Number.isFinite(result.width)
          ? result.width
          : undefined,
      naturalHeight: Number.isFinite(result.naturalHeight)
...
function normalizeMedia(media) {
  ...
  return {
    url,
    storageKey:
      typeof media.storageKey === "string" ? media.storageKey : undefined,
