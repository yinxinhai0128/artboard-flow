...
- 画布库：新建、打开、重命名、删除、批量删除、导入 JSON、导出 JSON。
- 无限画布：平移、缩放、缩放滑杆、重置视图、点阵/网格/空白背景、小地图。
- 节点：文本、图片、视频、配置四类节点。
- 节点交互：选择、多选、框选、拖拽、缩放、复制、粘贴、删除。
- 节点关系：左右连接点、拖拽连线、连线持久化、上下游高亮。
- 历史记录：撤销、重做覆盖节点、连线、视口和背景模式。
- 后端持久化：Fastify API + SQLite 保存项目、节点、连线、视口和背景。
...
- `types.ts`：项目、节点、连线、视口数据结构。
- `api.ts`：示例应用使用的项目持久化 API 客户端。
- `geometry.ts`：坐标变换、端口、连线路径等纯函数。

后续嵌入已有 AI 创作网站时，可以复用 `src/canvas/`，由宿主网站负责传入项目数据、保存项目数据，并在生成完成后调用节点更新能力插入图片或视频结果。
